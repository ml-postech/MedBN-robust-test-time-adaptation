...
def forward_and_adapt_eata(
    ...
    if current_model_probs is not None:
        ...
        updated_probs = update_model_probs(
            current_model_probs, outputs[filter_ids_1][filter_ids_2].softmax(1)
        )
    else:
        updated_probs = update_model_probs(
...
def check_model(model):
    ...
    assert has_any_params, "eata needs params to update: " "check which require grad"
    assert not has_all_params, (
        "eata should not update all params: " "check which require grad"
    )
